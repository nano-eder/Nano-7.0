...
Public Class clsGerarArqs
    ...
    '            strNome = Dir(.Item(I).Item(1).ToString, 30)
    '            strNome = Mid(strNome, 1, 30)

    '            strEnd = Dir(.Item(I).Item(2).ToString & " " & .Item(I).Item(3).ToString & " " & .Item(I).Item(4).ToString, 30)
    '            strEnd = Mid(strEnd, 1, 30)

    '            strCid = Dir(.Item(I).Item(5).ToString, 30)
    '            strCid = Mid(strCid, 1, 30)

    '            strBai = Dir(.Item(I).Item(6).ToString, 20)
    '            strBai = Mid(strBai, 1, 20)

    '            strUF = Dir(.Item(I).Item(7).ToString, 2)
    '            strUF = Mid(strUF, 1, 2)

    '            strTel = Dir(.Item(I).Item(8).ToString.Replace("|", "").Replace("Res", "").Replace("Com", "").Replace("Cel", "").Replace("Fax", "").Replace("Cont", "").Replace("Rec", "").Replace("RES", "").Replace("COM", "").Replace("CEL", "").Replace("FAX", "").Replace("CONT", "").Replace("REC", "").Replace("SAC", "").Replace("ID", ""), 15)
    '            strTel = Mid(strTel, 1, 15)

    '            strMail = .Item(I).Item(9).ToString.Replace("|", "").Replace("E-Mail", "").Replace("MSN", "").Replace("Skype", "").Replace("Site", "").Replace("EMAIL", "").Replace("SKYPE", "").Replace("SITE", "")
    ...
    '            RMREG4 &= "004" & Esq(strCodCli, 15).Replace(" ", "0") & strCpfOK & strNome & strEnd & strCid & strBai & strUF & strTel & strMail & strStatus.Trim & Dir("", 25) & strComplementar1 & strComplementar2 & strComplementar3 & strComplementar4 & vbCrLf
    '            intNumRegs += 1
    ...
    '        Dim strNome, strUM, strGrupo, strFab, strTipProd, strCodInterno, strCusto, strICMS, strTrib, strCST, strOriIcms, strIPPT, strPesa, strTipoFIA, strAtivo, strPreco, strNivelCombo, strCodBarra, strCodCF As String
    ...
    '                strGrupo = Dir(dt.Rows.Item(I).Item("Grupo").ToString, 30)
    '                strGrupo = Mid(strGrupo, 1, 30)

    '                strFab = Dir(dt.Rows.Item(I).Item("Fabricante").ToString, 30)
    '                strFab = Mid(strFab, 1, 30)


    '                'ITEM PARA REGISTRO RMREG6
    ...
    '                RMREG5 &= "005" & Dir(strCodCF, 20) & strNome & Dir(strUM, 3) & strCodInterno & strGrupo & strFab & strAtivo & strOriIcms & strCST & strTrib & strICMS & strIPPT & strCusto & strPesa & strTipoFIA & strTipProd & vbCrLf
    ...
    '    Dim strCod, strDesc, strPrazo, strForPag, strTipoPag, strTEF, strAtivo, strTroco, strParc As String
    '    strTroco = ""
    '    Dim I As Integer
    ...
    '        strDesc = Mid(dt.Rows.Item(I).Item(1).ToString, 1, 15)
    '        strDesc = Dir(strDesc, 20)
    ...
    '        RMREG9 &= "009" & strCod & Dir(strDesc, 30) & strDesc & strTipoPag & strForPag & strParc & strTEF & strAtivo & strTroco & vbCrLf
    '        intNumRegs += 1
    '    Next
    ...
    Public Shared Function EscreveArquivosCupom(ByVal parTipoArquivo As String, ByVal parNumRegistros As Integer) As Boolean
        ...
        RMREG1 = "001" & CNPJdoCliente.Trim & "REM" & strData & strHora & "0000" & "006"
